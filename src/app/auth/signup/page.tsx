...
import { Mail, Lock, Eye, EyeOff, Loader2, ArrowLeft, User, AlertCircle } from 'lucide-react';
import { getSupabaseClient } from '@/lib/supabase';
import { validateUsername, validateEmail, validatePassword } from '@/lib/contentModeration';
...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Validate full name (with content moderation)
    const nameValidation = validateUsername(fullName);
    if (!nameValidation.isValid) {
      setFormError(nameValidation.error || 'Invalid name');
      return;
    }
    
    // Validate email (with content moderation)
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <Link
        href="/"
        className="absolute top-6 left-6 flex items-center gap-2 text-[#6b6b70] hover:text-white transition-colors z-20"
      >
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[#6b6b70] mt-3">Create your account</p>
        </div>
        ...
        <div className="bg-[#111113]/80 backdrop-blur-xl border border-[#1f1f23]/50 rounded-2xl p-8">
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            {/* Full Name */}
            <div>
              <label className="block text-sm text-[#6b6b70] mb-2">Full Name</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6b6b70]" />
                <input
                  type="text"
                  value={fullName}
            ...
            <div>
              ...
              <p className="text-xs text-[#4a4a4e] mt-2">Must be at least 6 characters</p>
            </div>
          ...
          {/* Terms */}
          <p className="text-center text-[#4a4a4e] text-xs mt-6">
            By creating an account, you agree to our{' '}
            <Link href="/terms" className="text-[#6b6b70] hover:text-white transition-colors">
              Terms of Service
            </Link>{' '}
            and{' '}
            <Link href="/privacy" className="text-[#6b6b70] hover:text-white transition-colors">
              Privacy Policy
            </Link>
          </p>

          {/* Sign in link */}
          <p className="text-center text-[#6b6b70] mt-4">
